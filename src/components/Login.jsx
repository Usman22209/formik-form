...
const LoginPage = () => {
    const validationSchema=yup.object({
        ...
        password:yup.string().min(6).required("required") .matches(/^(?=.*[0-9|!@#$%^&*])/, 'Password must contain at least one number or one special character'),
    ...
    return (
        <div className="min-h-screen flex items-start sm:items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
            ...
            <div className="max-w-md w-full space-y-8">
                <div>
                    <h2 className="mt-6 text-center text-[42px] font-extrabold text-gray-900">
                ...
                <form className="mt-8 space-y-6" noValidate onSubmit={handleSubmit}>
                    ...
                    <div className="rounded-md shadow-sm -space-y-px">
                        <div>
                            ...
                            <input id="name" name="name" type="text" autoComplete="off" required className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Name" value={values.name} onChange={handleChange} onBlur={handleBlur} />
                            {touched.name && errors.name && <p className={` text-red-600 text-sm ${errors.name?"py-1":""} `} >{errors.name}</p>}
                        </div>
                        <div>
                            <label htmlFor="email-address" className="sr-only">Email address</label>
                            <input id="email-address" name="email" type="email" autoComplete="email" required className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Email address" value={values.email} onChange={handleChange} onBlur={handleBlur}/>
                           {touched.email && errors.email && <p className={` text-red-600 text-sm ${errors.name?"py-1":""} `} >{errors.email}</p>}
                        </div>
                        <div>
                           ...
                           {touched.password && errors.password && <p className={` text-red-600 text-sm ${errors.name?"py-1":""} `} >{errors.password}</p>}
                        </div>
                        <div>
                            ...
                            {touched.confirm_password && <p className={` text-red-600 text-sm ${errors.name?"py-1":""} `} >{errors.confirm_password}</p>}
                        </div>
                        <div>
                            <label htmlFor="gender" className="sr-only">Gender</label>
                            <select id="gender" name="gender" required className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" value={values.gender} onChange={handleChange} onBlur={handleBlur}>
